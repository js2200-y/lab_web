...
// argument: 함수를 호출할 때 함수를 전달하는 값.
// parameter: 전달받은 argument를 저장하기 위한 지역 변수. 함수 선언부에서 선언.

// 자바스크립트 함수는 파라미터의 타입을 검사하지 않음.
result = add('안녕하세용', 'hello');
console.log(result);

// 자바스크립트 함수는 파라미터 개수를 검사하지 않음.
result = add('이제훈', '표예진', '모범택시'); // 함수선언보다 더 많은 argument를 전달.
console.log(result);

result = add('이제훈'); // 함수 선언의 parameter 갯수보다 적은 argument를 전달.
console.log(result); // 전달하지 않은 파라미터의 값은 undifined가 됨.

result = add(1);
console.log(result); // NaN(Not a Number)

// 자바스크립트의 모든함수는 argument 이름의 프로퍼티를 가지고 있음.
