 ...
 */
 document.addEventListener('DOMContentLoaded', () => {
     ...
     const deleteReply = (e) => {
       ...
       const reqUrl = `/api/reply/${id}`; // 문자열 템플릿 사용할 경우는 작은따옴표X
       ...
       axios 
            ...
            .catch((error) => console.log(error)); // 실패 응답일 때 실행할 콜백 함수
     };
     
     // 댓글 수정 버튼 클릭을 처리하는 이벤트 리스너 콜백
     const updateReply = (e) => {
        // console.log(e.target);
        
        const replyId = e.target.getAttribute('data-id'); // 수정할 댓글 아이디
        const textAreaId = `textarea#replyText_${replyId}`; // 댓글 수정할 textarea 아이디
        ...
        axios
            ...
            .then((response) =>{ // 성공응답일 때 동작할 콜백을 등록.
                console.log(response);
     ...
     //btnModify.addEventListener('click', updateReply);
     ...
     const makeReplyElements = (data) => {
         ...
         for (let reply of data) {
             ...
             `;
             // 로그인한 사용자와 댓글 작성자가 같을때만 삭제, 수정 버튼을 보여주기
     ...
     btnReplyCreate.addEventListener('click', () => {
         ...
         // Ajax 요청에서 보낼 데이터
         // const data = {postId : postId, replyText : replyText, writer : writer};
         // 지역변수이름과 key값이 같은 경우는 아래와 같이 사용 가능함.
         ...
         axios
                .post(reqUrl, data) // Ajax POST 요청을 보냄.
                .then((response) => {
                    console.log(response);
