...
public class UserSignInController extends HttpServlet {
	...
	/**
	 * @see HttpServlet#doGet(HttpServletRequest requenst, HttpServletResponse response)
	 */
	@Override
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if (user == null) { // 유저네임 또는 비번이 일치하지 않는 경우- 로그인실패
		    // 로그인페이지로 이동(redirect)
		...
		session.setAttribute("signedInUser", user.getUsername()); // 세션 객체에 로그인 유저네임만 저장.
